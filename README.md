...
## Railway Volume і бекап

На Railway `server/data/users.json` всередині контейнера не переживає деплой або рестарт. Для production потрібен Volume.

1. Створи Railway Volume.
2. Змонтуй його в `/data`.
3. Задай змінну середовища `DATA_FILE=/data/users.json`.
4. Перезапусти сервіс і перевір, що в логах немає попередження `УВАГА: DATA_FILE не на Volume — дані зникнуть при наступному деплої`.

Для ручного бекапу доступний `GET /api/export` з тією самою Telegram-перевіркою `initData`, що й основне API. Ендпойнт повертає весь акаунт користувача JSON-файлом.

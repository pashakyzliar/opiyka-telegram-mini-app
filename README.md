...
- `CSP_MODE=report-only` — security-заголовки (CSP, `nosniff`, `Referrer-Policy`,
  `Permissions-Policy`). Типово лише звіти; після перевірки на всіх клієнтах
  Telegram (iOS, Android, Desktop, Web) перемкніть на `enforce`. `off` вимикає CSP.
